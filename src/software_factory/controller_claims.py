...
class ControllerClaimEvent:
    ...
    def from_mapping(cls, raw: object, *, field: str = "claim event") -> ControllerClaimEvent:
        ...
        if type(schema_version) is not int or schema_version != 1:
            raise ControllerClaimError(f"{field}.schema_version must be the integer 1")
...
def _read_event(path: Path) -> ControllerClaimEvent:
    try:
        ...
        raise ControllerClaimError(f"cannot read claim event {path}: {error}") from error
    try:
    ...
    return ControllerClaimEvent.from_mapping(raw, field=f"claim event {path.name}")
